...
namespace glm
{
   ...
   */
   ...
   /** Set Matrix from another dimensional matrix
    * 
    *
    * @param src        the matrix being accessed
    * @param result     the output matrix
    *
    * @return  a matrix copy from other matrix (only works for 3x3 --> 4x4)
    */
   template< typename DATA_TYPE, unsigned ROWS, unsigned COLS, unsigned ROWSIN, unsigned COLSIN >
   inline Matrix<DATA_TYPE, ROWS, COLS>& set(Matrix<DATA_TYPE, ROWS, COLS>& result, const Matrix<DATA_TYPE, ROWSIN, COLSIN>& src)
   {
       static_assert(ROWS == 4 && COLS == 4 && ROWSIN == 3 && COLSIN == 3);

       for (unsigned i = 0; i < ROWSIN; ++i)
       {
           for (unsigned j = 0; j < COLSIN; ++j)
           {
               result[i][j] = src[i][j];
           }
       }
   ...
   inline DEST_TYPE& setRot( DEST_TYPE& result, const Vec<DATA_TYPE, 3>& from, const Vec<DATA_TYPE, 3>& to )
   {
      ...
      else if ( Math::isEqual( cosangle, (DATA_TYPE)-1.0, epsilon ) )
      {
         ...
         normalize( glm::cross( axis, from, to_rot ) ); // setRot requires normalized vec
      ...
      else
      {
         ...
         normalize( glm::cross( axis, from, to ) ); // setRot requires normalized vec
         DATA_TYPE angle = Math::aCos( cosangle );
         return setRot( result, gmtl::AxisAngle<DATA_TYPE>( angle, axis ) );
      }
   }
